...
void Swapchain::Create_Swapchain(const Swapchain_Create_Info& info) {
    ...
    m_format = info.preferred_format;
    m_color_space = info.preferred_color_space;
    m_present_mode = info.present_mode;
    m_requested_min_image_count = info.min_image_count;

    VkSurfaceFormatKHR surface_format = Choose_Swap_Surface_Format(available_formats);
    ...
    uint32_t image_count = std::max(info.min_image_count, capabilities.minImageCount);
...
void Swapchain::Create_Sync_Objects() {
    ...
    m_image_render_finished.reserve(m_images.size());
    ...
    for (size_t i = 0; i < m_images.size(); ++i) {
        ...
        if (vkCreateSemaphore(m_vk_device, &semaphore_info, nullptr, &render_finished_raw) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create per-image render-finished semaphore");
        }
        m_image_render_finished.emplace_back(render_finished_raw, m_vk_device);
    }
}

uint32_t Swapchain::Acquire_Next_Image(uint64_t timeout) {
    // Wait for current frame's fence
    VkFence in_flight_fence = m_frame_sync[m_current_frame].in_flight.Get_Handle();
    vkWaitForFences(m_vk_device, 1, &in_flight_fence, VK_TRUE, UINT64_MAX);
...
void Swapchain::Present_Image(uint32_t image_index, const std::vector<VkSemaphore>& wait_semaphores) {
    VkSwapchainKHR swapchain_handle = m_swapchain.Get_Handle();
...
VkSemaphore Swapchain::Get_Render_Finished_Semaphore(uint32_t image_index) const {
    return m_image_render_finished.at(image_index).Get_Handle();
}
...
void Swapchain::Recreate(uint32_t new_width, uint32_t new_height) {
    ...
    info.min_image_count = m_requested_min_image_count;
